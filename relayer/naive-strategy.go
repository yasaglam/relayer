	...
	default:
		...
		eg.Go(func() error {
			return AddMessagesForSequences(egCtx, sp.Src, src, dst, srch, dsth, &msgs.Src, &msgs.Dst,
				srcChannel.ChannelId, srcChannel.PortId,
				srcChannel.Counterparty.ChannelId, srcChannel.Counterparty.PortId, srcChannel.Ordering)
		})

		// add messages for sequences on dst
		eg.Go(func() error {
			return AddMessagesForSequences(egCtx, sp.Dst, dst, src, dsth, srch, &msgs.Dst, &msgs.Src,
				srcChannel.Counterparty.ChannelId, srcChannel.Counterparty.PortId,
				srcChannel.ChannelId, srcChannel.PortId, srcChannel.Ordering)
		})
	...
	for _, seq := range sequences {
		...
		if err = retry.Do(func() error {
			recvMsg, timeoutMsg, err = src.ChainProvider.RelayPacketFromSequence(ctx, src.ChainProvider, dst.ChainProvider,
				uint64(srch), uint64(dsth), seq, dstChanID, dstPortID, srcChanID, srcPortID, src.PathEnd.ClientID, order)
			return err
		}, retry.Context(ctx), RtyAtt, RtyDel, RtyErr, retry.OnRetry(func(n uint, err error) {
			src.log.Debug(
...
func RelayPacket(ctx context.Context, src, dst *Chain, sp *RelaySequences, maxTxSize, maxMsgLength, seqNum uint64, srcChannel *chantypes.IdentifiedChannel) error {
	...
	for _, seq := range sp.Src {
		if seq == seqNum {
			...
			if err = retry.Do(func() error {
				recvMsg, timeoutMsg, err = src.ChainProvider.RelayPacketFromSequence(ctx, src.ChainProvider, dst.ChainProvider,
					uint64(srch), uint64(dsth), seq, dstChanID, dstPortID, srcChanID, srcPortID, src.ClientID(), srcChannel.Ordering)
				if err != nil {
					src.log.Warn(
		...
		if seq == seqNum {
			...
			if err = retry.Do(func() error {
				recvMsg, timeoutMsg, err = dst.ChainProvider.RelayPacketFromSequence(ctx, dst.ChainProvider, src.ChainProvider,
					uint64(dsth), uint64(srch), seq, srcChanID, srcPortID, dstChanID, dstPortID, dst.ClientID(), srcChannel.Ordering)
				if err != nil {
